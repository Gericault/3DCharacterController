extends State

@onready var stats: PlayerStats

@export var idle: State
@export var walk_state: State

func enter() -> void:
	parent.animation_tree.travel("freehand_fall", parent.ANIM_BLEND_SPEED * 2)
	stats = parent.player_stats
	

func exit() -> void:
	parent.momentum = Vector3.ZERO

func physics_process(_delta: float) -> State:
	
	var direction = parent.get_direction_from_input()
	
	if direction and parent.is_on_floor():
		return walk_state
	elif parent.is_on_floor():
		return idle
	else:
		#TODO make is so that you can only guide the fall slowly
		# (no 180 turns) check dot notation Vector3 + acos

		# stats.air_control allows a % control over airmovement, as well as 1/3 turnspeed
		var fall_momentum = (parent.momentum * (1.0 - stats.air_control)) + (direction * stats.air_control)
		# cheat and add move gravity on fall for less floaty physics
		parent.velocity += ((parent.get_gravity() * _delta) * 0.5)
		parent.move_character(fall_momentum, stats.turn_speed / 3)
		return null
